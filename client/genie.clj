...
;; 1 - error occurred (ex)
;; 2 - error occurred
;; 3 - break (C-c) detected, in kill-script. -> removed, causes hang.
;; 4 - error occurred
;; 5 - exception in main
;; 6 - error occurred (root-ex)
;; 130 - C-c pressed, break (in genie itself, but also in script)
;; 137 - got kill -9 (in genie itself)
;; 143 - got kill (-3?, in genie itself, also script)
    ...
    (try
      ...
      (loop [it 0]
        (let [res (read-print-result in)]
          (when (:error res)
            (warn "Error occurred, exit 4: " res)
            (System/exit 4))
          (when (:need-input res)
            ...
            (let [lines (read-lines opt *in*)]
              ...
              (recur (inc it))))))
      (catch Exception e
        (warn "Caught exception, exit 2: " e)
        (System/exit 2))
      (finally
...
(defn kill-script
  ...
  (if-let [{:keys [session eval-id] :as admin-session} @session-atom]
    (do
      ...
      ;; 2024-02-22: Do not exit here, causes a script hang.
      )
